# Week 11

Chapters ([Computer Networking A Top-Down Approach](https://annas-archive.gl/md5/f9dfec894d8462e58dc9140e9c75fa9b)):
- 3 Transport Layer
    - 3.5 Connection-Oriented Transport: TCP
        - 3.5.1 The TCP Connection
        - 3.5.2 TCP Segment Structure
        - 3.5.3 Round-Trip Time Estimation and Timeout
        - 3.5.4 Reliable Data Transfer
        - 3.5.5 Flow Control
        - 3.5.6 TCP Connection Management
    - 3.6 Principles of Congestion Control
        - 3.6.1 The Causes and the Costs of Congestion
        - 3.6.2 Approaches to Congestion Control
    - 3.7 TCP Congestion Control
        - 3.7.1 Classic TCP Congestion Control
        - 3.7.2 Network-Assisted Explicit Congestion Notification and Delayed-based Congestion Control
        - 3.7.3 Fairness

Videos:
- [3.5 Connection-Oriented Transport](https://youtu.be/UYJP-6mhF6E)
    - 3.5.1 The TCP Connection
    - 3.5.2 TCP Segment Structure
    - 3.5.3 Round-Trip Time Estimation and Timeout
    - 3.5.4 Reliable Data Transfer
- [3.5 Connection-Oriented Transport](https://youtu.be/E4I6t0mI_is)
    - 3.5.5 Flow Control
    - 3.5.6 TCP Connection Management
- [3.6 Principles of Congestion Control](https://youtu.be/Fm92xvIp6JY)
    - 3.6.1 The Causes and the Costs of Congestion
    - 3.6.2 Approaches to Congestion Control
- [3.7 TCP Congestion Control](https://youtu.be/cIHiSR4j3g4)
    - 3.7.1 Classic TCP Congestion Control
    - 3.7.2 Network-Assisted Explicit Congestion Notification and Delayed-based Congestion Control
    - 3.7.3 Fairness